...
func (v *verifySHA1) SetServerInfo(s *ServerInfo) {
	v.ServerInfo = *s
}

func (v *verifySHA1) GetServerInfo() (s *ServerInfo) {
	return &v.ServerInfo
}
...
func (v *verifySHA1) PreEncrypt(data []byte) (encryptedData []byte, err error) {
	...
	if !v.hasSentHeader {
		...
		v.buffer.Write(v.otaConnectAuth(data[:v.AddrLen]))
		v.hasSentHeader = true
		dataLength -= v.AddrLen
		offset += v.AddrLen
	}
